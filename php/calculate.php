...
<main>
    ...
    <form id="co2Form" method="POST" action="calculate.php">
        ...
        <button type="submit" id="calculation">Calculate</button>
...
<script>
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            totalEmissionEl.dataset.mealEmission = result.meal_emission ?? 'N/A';
            totalEmissionEl.dataset.electricityEmission = result.electricity_emission ?? 'N/A';
            totalEmissionEl.dataset.plasticEmission = result.plastic_emission ?? 'N/A';
            totalEmissionEl.dataset.travelEmission = result.travel_emission ?? 'N/A';
    ...
    document.getElementById('downloadPdfBtn').addEventListener('click', () => {
        // Grab the displayed values
        const username = document.getElementById('username').value.trim();
        if (!username) {
            alert('Please enter your username before downloading the PDF.');
            return;
        }
        
        if (resultBlock.style.display === 'none') {
            alert('Please calculate the emissions first before downloading PDF.');
            return;
        }

        const mealEmission = totalEmissionEl.dataset.mealEmission || 'N/A';
        const electricityEmission = totalEmissionEl.dataset.electricityEmission || 'N/A';
        const plasticEmission = totalEmissionEl.dataset.plasticEmission || 'N/A';
        const travelEmission = totalEmissionEl.dataset.travelEmission || 'N/A';
        const totalEmission = totalEmissionEl.textContent || 'N/A';
        ...
        doc.setFontSize(18);
        doc.text("🌍 CO₂ Emission Report", 14, 22);

        doc.setFontSize(12);
        doc.text(`Username: ${username}`, 14, 32);
        doc.text(`Date/Time: ${dateTime}`, 14, 40);

        // Prepare table data
        const tableColumn = ["Category", "Emission (kg CO₂)"];
        const tableRows = [
            ["Meal", mealEmission],
        ...
        doc.save(`CO2_Emission_Report_${username}_${Date.now()}.pdf`);
    });
    ...
    <?php if ($result && !isset($result['error'])): ?>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            totalEmissionEl.dataset.mealEmission = <?= json_encode($result['meal_emission'] ?? 'N/A') ?>;
            totalEmissionEl.dataset.electricityEmission = <?= json_encode($result['electricity_emission'] ?? 'N/A') ?>;
            totalEmissionEl.dataset.plasticEmission = <?= json_encode($result['plastic_emission'] ?? 'N/A') ?>;
            totalEmissionEl.dataset.travelEmission = <?= json_encode($result['travel_emission'] ?? 'N/A') ?>;
